...
export function DashboardCharts({ logs, selectedDate }: DashboardChartsProps) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#0F0F0F] border border-white/10 rounded-lg p-4 shadow-xl font-sans select-none">
          <p className="text-sm text-white/70 font-bold mb-2">{label}</p>
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 select-none">
      ...
      <div className="bg-[#141414] rounded-xl p-6 border border-white/10 shadow-lg flex flex-col justify-between">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-4 border-b border-white/10 mb-4">
          ...
          <span className="text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded font-mono uppercase tracking-widest font-bold">
            Vueltas Despachadas
          </span>
        </div>
